...
def test_find_breaks():
    ...
    breaks = Periodogram.find_breaks(x, y)
    assert 0 == len(breaks[0])  # no breaks

    breaks = Periodogram.find_breaks(x, y, break_value=2)
...
def test_circulation_and_circulation_metrics():
    ...
    data = Periodogram.circulation(x, y)
    assert 3.0 == data[0][0]
    assert 7.0 == data[1][0]
    assert 4.0 == data[2][0]
    ...
    metrics = Periodogram.circulation_metrics(data)
...
def test_overlap():
    assert 1.0 == Periodogram.overlap([1, 3], [2, 4])
    assert 0.0 == Periodogram.overlap([1, 2], [3, 4])
    assert 0.0 == Periodogram.overlap([1, 2], [2, 3])
    assert 2.0 == Periodogram.overlap([1, 10], [8, 12])

    assert 0 < Periodogram.overlap([1, 2], [2, 3], delta=0.1)
    assert 0 == Periodogram.overlap([1, 2], [3, 4], delta=0.1)
    assert 0 < Periodogram.overlap([1, 2], [3, 4], delta=1.1)


def test_overlap_list():
    data = Periodogram.overlap_list([[1, 3], [10, 11]], [[2, 4], [20, 21]])
    assert 1 == len(data)
    assert [1, 3] == data[0]

    data = Periodogram.overlap_list([[1, 2], [10, 11]], [[3, 4], [20, 21]])
    assert 0 == len(data)

    data = Periodogram.overlap_list([[1, 3], [10, 12]], [[2, 4], [11, 21]])
    assert 2 == len(data)
    assert [1, 3] == data[0]
    assert [10, 12] == data[1]

    data = Periodogram.overlap_list([[1, 2], [10, 11]], [[2, 3], [20, 21]])
    assert 0 == len(data)
